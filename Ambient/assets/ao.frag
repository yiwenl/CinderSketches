...
{	
    ...
    vec2 diff = vTexCoord0 - 0.5;
    diff.x *= uRatio;
    float l = length(diff);
    float offset = 0.0;
    float d = smoothstep(0.3 + offset, 0.65 + offset, l);

    float ao = ssao();
    ao = pow(ao, 2.0);
    float aoOffset = smoothstep(0.1, 0.0, l);
    ao = mix(ao, 1.0, 0.5 + aoOffset * 0.5);
    ao = mix(ao, 1.0, 0.8);
    ...
    color = mix(color, vec3(1.0), d * 0.9);
